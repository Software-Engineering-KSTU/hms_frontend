...
class CustomPrettyDioLogger extends Interceptor {
  ...
  CustomPrettyDioLogger({
    ...
    this.filter,
    this.enabled = true,
    this.sensitiveKeys = const ['password', 'accessToken', 'refreshToken', 'authorization'],
  ...
  void _printPrettyMap(
    ...
    for (var index = 0; index < data.length; index++) {
      ...
      // --- КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Проверка на чувствительный ключ ---
      final keyName = data.keys.elementAt(index).toString();
      if (sensitiveKeys.map((k) => k.toLowerCase()).contains(keyName.toLowerCase())) {
      ...
      if (value is Map) {
        ...
        // ... (логика для примитивных значений, используем уже потенциально замаскированное value)
  ...
  // Вспомогательный метод для проверки и печати (заменяет _printKV)
  void _printMaskedKV(String? key, Object? v) {
    Object? valueToPrint = v;

    // Проверка, является ли ключ чувствительным (чувствительность не зависит от регистра)
    if (key != null && sensitiveKeys.map((k) => k.toLowerCase()).contains(key.toLowerCase())) {
      valueToPrint = sensitiveValueMask;
    }

    final pre = '╟ $key: ';
    final msg = valueToPrint.toString();
  ...
  void _printMapAsTable(Map? map, {String? header}) {
    ...
    for (final entry in map.entries) {
      _printMaskedKV(entry.key.toString(), entry.value);
    }
    _printLine('╚');
  }
}
